...
app = FastAPI(
    title="AI Crypto Oracle – Autonomous Swarm 2025 🔥",
    version="1.1.0",
...
async def startup_event():
    ...
    logger.info(f"Telegram Alerts: {'✓' if send_alert else '✗'}")
    logger.info(f"ML Predictor: {'✓' if get_ml_signal else '✗'}")
    logger.info(f"Analytics: {'✓' if analytics_logger else '✗'}")
    logger.info(f"Portfolio: {'✓' if portfolio else '✗'}")
    logger.info(f"Flash Loan Protection: {'✓' if flash_loan_protector else '✗'}")
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={"detail": "Oracle resilient – empire endures 🔥", "error": str(exc)}
    )
...
async def background_trading_swarm():
    ...
    logger.info("🤖 Background trading swarm activated")
